		...
		Sections: []APIDocSection{
			...
			},
			{
				...
				Endpoints: []APIDocEndpoint{
					...
					{Method: "GET", Path: "/api/admin/tts", Summary: "TTS sidecar settings", Auth: "admin"},
					{Method: "PUT", Path: "/api/admin/tts", Summary: "Update TTS settings", Auth: "admin"},
					{Method: "POST", Path: "/api/admin/tts/test", Summary: "Ping TTS sidecar", Auth: "admin"},
					{Method: "GET", Path: "/api/tts/status", Summary: "Whether server TTS is enabled", Auth: "required"},
					{Method: "GET", Path: "/api/tts/voices", Summary: "List TTS voices", Auth: "required"},
					{Method: "POST", Path: "/api/tts/synthesize", Summary: "Synthesize speech via the TTS sidecar", Auth: "required", Body: "{text, voice, speed}", Response: "audio bytes"},
					{Method: "GET", Path: "/api/tts/prefs", Summary: "Your narration preferences and schedule", Auth: "required"},
					{Method: "PUT", Path: "/api/tts/prefs", Summary: "Save narration preferences", Auth: "required", Body: "{voice, speed, schedEnabled, schedStart, schedEnd}"},
					{Method: "GET", Path: "/api/tts/jobs", Summary: "List your audiobook jobs (admins: all=1)", Auth: "required", Query: "all"},
					{Method: "POST", Path: "/api/tts/jobs", Summary: "Queue a whole-book narration job", Auth: "required", Body: "{bookId, voice?, speed?}"},
					{Method: "POST", Path: "/api/tts/jobs/{id}/cancel", Summary: "Cancel a queued or running job", Auth: "required"},
					{Method: "POST", Path: "/api/tts/jobs/{id}/retry", Summary: "Requeue a finished, failed, or cancelled job", Auth: "required"},
					{Method: "DELETE", Path: "/api/tts/jobs/{id}", Summary: "Delete a finished job record", Auth: "required"},
