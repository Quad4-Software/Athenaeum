...
func Parse(args []string) (Config, error) {
	...
	sandbox := fs.String("sandbox", Env("ATHENAEUM_SANDBOX", "try"), "Linux sandbox mode: off, try, or strict")
...
func PrintHelp(w io.Writer) {
	...
	term.Fprintf(w, "  %s self-check [flags]\n", name)
	term.Fprintln(w)
	...
	printFlag(w, "--sandbox", "off, try, or strict Linux Landlock/seccomp (default try)")
	...
	printFlag(w, "--self-check", "verify dirs, database, and HTTP then exit")
	printFlag(w, "--help", "show this help")
	term.Fprintln(w)
	term.Fprintln(w, term.Header(w, "Commands:"))
	term.Fprintf(w, "  %-22s %s\n", term.Flag(w, "self-check"), "same as --self-check (alias: doctor)")
	term.Fprintf(w, "  %-22s %s\n", term.Flag(w, "users"), "manage local accounts without starting the server")
	term.Fprintf(w, "  %-22s %s\n", term.Flag(w, "version"), "print version and exit")
	term.Fprintln(w)
